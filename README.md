# 🧪 TioElvis Lab

A personal development blog and project documentation platform built to showcase technical projects across multiple programming languages. Inspired by GitBook's structure

## 🎯 Purpose

TioElvis Lab serves as a personal knowledge base where I document:

- **Multi-language projects** (JavaScript, Python, C, and more)
- **Step-by-step development processes** with detailed sections
- **Code implementations** with real-world examples
- **Technical learnings** organized in a structured, easy-to-navigate format

## 🛠️ Tech Stack

### Frontend

- **[Next.js 14+](https://nextjs.org/)** (App Router) - React framework with server components
- **[TypeScript](https://www.typescriptlang.org/)** - Type-safe development
- **[Tailwind CSS](https://tailwindcss.com/)** - Utility-first styling

### Backend & Database

- **[Prisma ORM](https://www.prisma.io/)** - Type-safe database client
- **[Neon.tech](https://neon.tech/)** - Serverless PostgreSQL database

### Deployment

- **[Vercel](https://vercel.com/)** - Hosting and CI/CD

## 🤝 Contributing

This is a personal project, but suggestions and feedback are welcome! Feel free to:

- Open issues for bugs or feature requests
- Fork the repo for your own use
- Share ideas for improvements

## 📄 License

MIT License - feel free to use this project as inspiration for your own portfolio or documentation site.

## 🙏 Credits

This project was built with inspiration and help from:

- **[GitBook](https://www.gitbook.com/)** - For the hierarchical documentation structure concept
- **[Anthropic Claude](https://www.anthropic.com/)** - AI assistance during development
- **[shadcn/ui](https://github.com/shadcn-ui/ui)** - AI assistance during development
- **[jstack](https://github.com/upstash/jstack)** - AI assistance during development

---

Built with ❤️ and lots of ☕ by TioElvis

```
            ________o8A888888o_
        _o888888888888K_1888888o
                  ~~~+8888888888o
                      ~8888888888
                      o88888888888
                     o8888888888888
                   _8888888888888888
                  o888888888888888888_
                 o88888888888888888888_
                _8888888888888888888888_
                888888888888888888888888_
                8888888888888888888888888
                88888888888888888888888888
                88888888888888888888888888
                888888888888888888888888888
                ~88888888888888888888888888_
                 488888888888888888888888888
                  888888888888888888888888888
                   888888888888888888888888888_
                   ~8888888888888888888888888888
                     +88888888888888888888~~~~~
                      ~=888888888888888888o
               _=oooooooo888888888888888888
                _o88=8888==~88888888===8888
                ~   =~~ _o88888888=      ~~~
                        ~ o8=~88=~
```
